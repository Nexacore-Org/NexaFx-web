...
import {
  ...
  OfflineError,
  ApiError,
  DirectApiUnreachableError,
  isOfflineError,
  isDirectApiUnreachableError,
} from "../lib/api-client";
...
describe("apiClient", () => {
  ...
  describe("direct (non-proxied) call network failures", () => {
    it("throws DirectApiUnreachableError when a direct call fails while online (likely CORS/unreachable backend)", async () => {
      mockFetch.mockRejectedValueOnce(new TypeError("Failed to fetch"));

      await expect(apiClient("/test", { useProxy: false })).rejects.toThrow(
        DirectApiUnreachableError,
      );
    });

    it("DirectApiUnreachableError message points at CORS/NEXT_PUBLIC_API_URL as the likely cause", async () => {
      mockFetch.mockRejectedValueOnce(new TypeError("Failed to fetch"));

      await expect(apiClient("/test", { useProxy: false })).rejects.toThrow(
        /CORS|NEXT_PUBLIC_API_URL/,
      );
    });

    it("still throws the generic offline error for a direct call when navigator.onLine is false", async () => {
      navigator.onLine = false;
      mockFetch.mockRejectedValueOnce(new TypeError("Failed to fetch"));
    ...
    it("does not wrap a proxied call failure in DirectApiUnreachableError", async () => {
      mockFetch.mockRejectedValueOnce(new TypeError("Failed to fetch"));

      await expect(apiClient("/test", { useProxy: true })).rejects.not.toThrow(
        DirectApiUnreachableError,
      );
    });

    it("isDirectApiUnreachableError and isOfflineError do not misclassify each other", () => {
      const directError = new DirectApiUnreachableError(
        "https://api.example.com/test",
      );
      const offlineError = new OfflineError();

      expect(isDirectApiUnreachableError(directError)).toBe(true);
      expect(isOfflineError(directError)).toBe(false);

      expect(isOfflineError(offlineError)).toBe(true);
      expect(isDirectApiUnreachableError(offlineError)).toBe(false);
    });
  });
});
